...
pub async fn check_ip_ban(
    ...
    let tp = s
        .settings
        .read()
        .map(|s| s.trust_proxy_headers)
        .unwrap_or(false);
    if let Some(ip) = client_ip(&req, tp) {
        if s.ip_ban.read().unwrap().is_banned(&ip) {
...
pub async fn require_auth(
    ...
    let tp = s
        ...
        .map(|s| s.trust_proxy_headers)
        .unwrap_or(false);
    let (username, token) = extract_credentials(req.headers()).map_err(|e| {
        record(&s, &req, tp);
        e
    })?;
    let user = resolve_user(&s.db, &username, &token).await.map_err(|e| {
        record(&s, &req, tp);
        e
    })?;
    if let Some(ip) = client_ip(&req, tp) {
...
fn record(s: &Arc<AppState>, req: &Request, trust_proxy: bool) {
    if let Some(ip) = client_ip(req, trust_proxy) {
        if s.ip_ban.write().unwrap().record_failure(&ip) {
            log::warn!("IP {ip} blacklisted");
            let _ = ip_ban::save_blacklist(
                &s.blacklist_path,
